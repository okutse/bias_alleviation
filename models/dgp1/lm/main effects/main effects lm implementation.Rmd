...
```{r}
rm(list = ls())
load("../ThesisResults/data/df_one.RData")
load("../ThesisResults/data/df_two.RData")
load("../ThesisResults/data/df_three.RData")
load("../ThesisResults/data/df_four.RData")
...
# get the results under each  simulated data set
  onea = parLapply(cl, dsets1, lm_one)
  oneb = parLapply(cl, dsets2, lm_one)
  onec = parLapply(cl, dsets3, lm_one)
  oned = parLapply(cl, dsets4, lm_one)
stopCluster(cl)
```
...
## compute results across all 1000 data sets
  twoa = parLapply(cl, dsets1, lm_two)
  twob = parLapply(cl, dsets2, lm_two)
  twoc = parLapply(cl, dsets3, lm_two)
  twod = parLapply(cl, dsets4, lm_two)
stopCluster(cl)
```
...
## compute over 1000 data sets
  threea = parLapply(cl, dsets1, lm_three)
  threeb = parLapply(cl, dsets2, lm_three)
  threec = parLapply(cl, dsets3, lm_three)
  threed = parLapply(cl, dsets4, lm_three)
stopCluster(cl)
```
...
```{r lm}
linear_model = bind_rows(list("n = 500, SD = 1" = full, "n = 500, SD = 1" = obs, "n = 500, SD = 1"= obs_m, "n = 500, SD = 45" = full2, "n = 500, SD = 45" = obs2, "n = 500, SD = 45" = obs_m2, "n = 2000, SD = 1" = full3, "n = 2000, SD = 1" = obs3, "n = 2000, SD = 1" = obs_m3, "n = 2000, SD = 45" = full4, "n = 2000, SD = 45"=  obs4, "n = 2000, SD = 45" = obs_m4), .id = "Data generating values") 
kable(linear_model, format = "latex", caption = "Main effects linear regression model results averaged across n = 1000 datasets under full, observed, and observed modified analysis")


## the order of the rows starts with n = 500 
write.csv(linear_model, file = "G:\\Shared drives\\amos\\ThesisResults\\[2]_main_effects_linear\\main_effects_linear.csv", row.names = FALSE)
```

## SECTION B

NOTE: Run this section separately.

- Add some new cases here to make plots of the variance explained

```{r}
##load all the data sets for this new simulations
rm(list = ls())

load("/Users/aokutse/Library/CloudStorage/GoogleDrive-amos_okutse@brown.edu/Shared drives/amos/ThesisResults/data/df_oneb.RData")
load("/Users/aokutse/Library/CloudStorage/GoogleDrive-amos_okutse@brown.edu/Shared drives/amos/ThesisResults/data/df_twob.RData")
load("/Users/aokutse/Library/CloudStorage/GoogleDrive-amos_okutse@brown.edu/Shared drives/amos/ThesisResults/data/df_threeb.RData")
load("/Users/aokutse/Library/CloudStorage/GoogleDrive-amos_okutse@brown.edu/Shared drives/amos/ThesisResults/data/df_fourb.RData")
...
##------------------------------------------------------------------------------
## full
full <- c(n = nrow(df_oneb), ate = mean(oneaa$ATE_adjusted), sd = sd(oneaa$ATE_adjusted), bias = mean(oneaa$bias_adjusted))
full
## observed
obs <- c(n = nrow(subset(df_oneb, R == 1)), ate = mean(twoaa$ATE_adjusted), sd = sd(twoaa$ATE_adjusted), bias = mean(twoaa$bias_adjusted))
obs
## observed modified
obs_m <- c(n = nrow(subset(df_oneb, R == 1)), ate = mean(threeaa$ATE_adjusted), sd = sd(threeaa$ATE_adjusted), bias = mean(threeaa$bias_adjusted))
obs_m
...
## case 2 [n = 500, sd = 68]
##------------------------------------------------------------------------------
full2 <- c(n = nrow(df_twob), ate = mean(onebb$ATE_adjusted), sd = sd(onebb$ATE_adjusted), bias = mean(onebb$bias_adjusted))
full2
## observed
obs2 <- c(n = nrow(subset(df_twob, R == 1)), ate = mean(twobb$ATE_adjusted), sd = sd(twobb$ATE_adjusted), bias = mean(twobb$bias_adjusted))
obs2
## observed modified
obs_m2 <- c(n = nrow(subset(df_twob, R == 1)), ate = mean(threebb$ATE_adjusted), sd = sd(threebb$ATE_adjusted), bias = mean(threebb$bias_adjusted))
obs_m2
...
## case 3 [n = 2000, sd = 23]
##------------------------------------------------------------------------------
full3 <- c(n = nrow(df_threeb), ate = mean(onecc$ATE_adjusted), sd = sd(onecc$ATE_adjusted), bias = mean(onecc$bias_adjusted))
full3
## observed
obs3 <- c(n = nrow(subset(df_threeb, R == 1)), ate = mean(twocc$ATE_adjusted), sd = sd(twocc$ATE_adjusted), bias = mean(twocc$bias_adjusted))
obs3
## observed modified
obs_m3 <- c(n = nrow(subset(df_threeb, R == 1)), ate = mean(threecc$ATE_adjusted), sd = sd(threecc$ATE_adjusted), bias = mean(threecc$bias_adjusted))
obs_m3

##------------------------------------------------------------------------------
## case 4 [n = 2000, sd = 68]
##------------------------------------------------------------------------------
full4 <- c(n = nrow(df_fourb), ate = mean(onedd$ATE_adjusted), sd = sd(onedd$ATE_adjusted), bias = mean(onedd$bias_adjusted))
full4
## observed
obs4 <- c(n = nrow(subset(df_fourb, R == 1)), ate = mean(twodd$ATE_adjusted), sd = sd(twodd$ATE_adjusted), bias = mean(twodd$bias_adjusted))
obs4
## observed modified
obs_m4 <- c(n = nrow(subset(df_fourb, R == 1)), ate = mean(threedd$ATE_adjusted), sd = sd(threedd$ATE_adjusted), bias = mean(threedd$bias_adjusted))
obs_m4

linear_model = bind_rows(list("n = 500, SD = 23" = full, "n = 500, SD = 23" = obs, "n = 500, SD = 23"= obs_m, "n = 500, SD = 68" = full2, "n = 500, SD = 68" = obs2, "n = 500, SD = 68" = obs_m2, "n = 2000, SD = 23" = full3, "n = 2000, SD = 23" = obs3, "n = 2000, SD = 23" = obs_m3, "n = 2000, SD = 68" = full4, "n = 2000, SD = 68"=  obs4, "n = 2000, SD = 68" = obs_m4), .id = "Data generating values") 
kable(linear_model, format = "latex", caption = "version 2")


## the order of the rows starts with n = 500 
write.csv(linear_model, file = "/Users/aokutse/Library/CloudStorage/GoogleDrive-amos_okutse@brown.edu/Shared drives/amos/ThesisResults/[2]_main_effects_linear/main_effects_linearb.csv", row.names = FALSE)
```
