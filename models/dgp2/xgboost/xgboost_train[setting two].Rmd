---
title: "eXtreme Gradient Boosted Trees Training (xGBoost) under simulation setting 2"
...
## data
train <- s_twoa %>% dplyr::select(y, A, x1, x2, x3, x4)
...
## data
train <- s_twob %>% dplyr::select(y, A, x1, x2, x3, x4)
...
## data
train <- s_twoc %>% dplyr::select(y, A, x1, x2, x3, x4)
...
## data
train <- s_twod %>% dplyr::select(y, A, x1, x2, x3, x4)
...
## data
train1 <- s_twoa %>% dplyr::select(y, A, x1, x2, x3, x4, R)
train = train1[, -7]
...
## data
train1 <- s_twob %>% dplyr::select(y, A, x1, x2, x3, x4, R)
train = train1[, -7]
...
## data
train1 <- s_twoc %>% dplyr::select(y, A, x1, x2, x3, x4, R)
train = train1[, -7]
# XGBoost model specification
xgboost_model <- 
  parsnip::boost_tree(
