---
title: "eXtreme Gradient Boosted Trees Training (xGBoost) under simulation setting 1"
...
## data
train <- s_onea %>% dplyr::select(y, A, x1, x2, x3, x4)
...
xgboost_tuned <- tune::tune_grid(
  ...
  resamples = cv_folds,
  grid = xgboost_grid,
  metrics = yardstick::metric_set(yardstick::rmse), ## can use mae and rsq
...
## data
train <- s_oneb %>% dplyr::select(y, A, x1, x2, x3, x4)
...
## data
train <- s_onec %>% dplyr::select(y, A, x1, x2, x3, x4)
...
## data
train <- s_oned %>% dplyr::select(y, A, x1, x2, x3, x4)
...
## data
train1 <- s_onea %>% dplyr::select(y, A, x1, x2, x3, x4, R)
train = train1[, -7]
...
## data
train1 <- s_oneb %>% dplyr::select(y, A, x1, x2, x3, x4, R)
train = train1[, -7]
...
xgboost_tuned <- tune::tune_grid(
  ...
  resamples = cv_folds,
  grid = xgboost_grid,
  metrics = yardstick::metric_set(yardstick::rmse), ## can use mae and rsq, rmse
  control = tune::control_grid(verbose = TRUE)
)
...
## data
train1 <- s_onec %>% dplyr::select(y, A, x1, x2, x3, x4, R)
train = train1[, -7]
# XGBoost model specification
xgboost_model <- 
  parsnip::boost_tree(
