...
subtitle: "Correctly Specified Linear regression Model"
author: "Amos Okutse"
...
model_onea <- function(df){
  # fit linear model for all individuals
  lm_all <- linear_reg() %>% 
  set_mode("regression") %>% 
  set_engine("lm") %>% 
...
model_onec <- function(df){
  
  ## fit model for individuals with R == 1 and make predictions for everyone
  lm_c <- linear_reg() %>% 
  set_mode("regression") %>% 
  set_engine("lm") %>% 
  fit(formula = y ~ A + z1 + z2 + z3 + z4, data = base::subset(df, R == 1))
## set A = 0 and generate predictions for everyone
  df_A0 <- df
  df_A0$A <- 0
  pred_A0 <- predict(lm_c, df_A0)
## set A = 1 and generate predictions for everyone
  df_A1 <- df
  df_A1$A <- 1
  pred_A1 <- predict(lm_c, df_A1)
## compute the ATE
  ATE_adjusted = mean(pred_A1$.pred - pred_A0$.pred)
## compute the biases in absolute values
  bias_adjusted = ATE_adjusted - 50
...
kable(sim1_model_one, format = "pipe", caption = "Linear regression model results using the true outcome model averaged across n = 1000 datasets [Model One]")

## save file on disk
write.csv(sim1_model_one, file = "/Users/aokutse/Library/CloudStorage/GoogleDrive-amos_okutse@brown.edu/Shared drives/amos/ThesisResults/data/dgp2/sim1_model_one.csv", row.names = FALSE)
```
...
model_twoa <- function(df){
  # fit linear model for all individuals
  lm_all <- linear_reg() %>% 
  set_mode("regression") %>% 
  set_engine("lm") %>% 
  fit(formula = y ~ A + z2 + z3 + z4, data = df) #adjustment model has no z1 here!!
## set A = 0 and generate predictions for everyone
  df_A0 <- df
  df_A0$A <- 0
  pred_A0 <- predict(lm_all, df_A0)
...
- Model 2 when the analysis is based on only treated observations and the resulting model extended to all observations in the data set including the untreated observations and the adjustment model does not include $Z_1$.

```{r}
## observed modified analysis under model 1
model_twoc <- function(df){
  
  ## fit model for individuals with R == 1 and make predictions for everyone
  lm_c <- linear_reg() %>% 
  set_mode("regression") %>% 
  set_engine("lm") %>% 
  fit(formula = y ~ A + z2 + z3 + z4, data = base::subset(df, R == 1))
## set A = 0 and generate predictions for everyone
  df_A0 <- df
  df_A0$A <- 0
  pred_A0 <- predict(lm_c, df_A0)
...
kable(sim1_model_two, format = "pipe", caption = "Linear regression model results using the true outcome model averaged across n = 1000 datasets [Simulation 1 Model Two]")

## write simulation 1 model 2 results to table
write.csv(sim1_model_two, file = "/Users/aokutse/Library/CloudStorage/GoogleDrive-amos_okutse@brown.edu/Shared drives/amos/ThesisResults/data/dgp2/sim1_model_two.csv", row.names = FALSE)
```
...
-write out resulting model here!!
...
kable(sim2_model_one, format = "pipe", caption = "Linear regression model results using the true outcome model averaged across n = 1000 datasets [Simulation 2 Model one]")

## save file on disk
write.csv(sim2_model_one, file = "/Users/aokutse/Library/CloudStorage/GoogleDrive-amos_okutse@brown.edu/Shared drives/amos/ThesisResults/data/dgp2/sim2_model_one.csv", row.names = FALSE)
```
...
kable(sim2_model_two, format = "pipe", caption = "Linear regression model results using the true outcome model averaged across n = 1000 datasets [Simulation 2 Model Two]")


## save simulation 2 model two results file on disk
write.csv(sim2_model_two, file = "/Users/aokutse/Library/CloudStorage/GoogleDrive-amos_okutse@brown.edu/Shared drives/amos/ThesisResults/data/dgp2/sim2_model_two.csv", row.names = FALSE)
```
...
kable(sim3_model_one, format = "pipe", caption = "Linear regression model results using the true outcome model averaged across n = 1000 datasets [Simulation 3 Model One]")

## save file on disk
write.csv(sim3_model_one, file = "/Users/aokutse/Library/CloudStorage/GoogleDrive-amos_okutse@brown.edu/Shared drives/amos/ThesisResults/data/dgp2/sim3_model_one.csv", row.names = FALSE)
```
...
kable(sim3_model_two, format = "pipe", caption = "Linear regression model results using the true outcome model averaged across n = 1000 datasets [Simulation 3 Model Two]")

## write simulation 3 model 2 results to table
write.csv(sim3_model_two, file = "/Users/aokutse/Library/CloudStorage/GoogleDrive-amos_okutse@brown.edu/Shared drives/amos/ThesisResults/data/dgp2/sim3_model_two.csv", row.names = FALSE)
```
